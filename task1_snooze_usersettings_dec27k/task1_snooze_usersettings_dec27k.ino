...
switch(index){
   case 0: // SET 12/24-hour
    ...
    welcome_updatepage();//goback
    //time_12();
    sw=0;///return to go back to welcome page .
    break;
...
void beep_alarm() {
  ...
  while (alarm_en) {
    ...
    if(buttonPressed(SW3)){//sw3 snooze button 
      noTone(buzzer);
      set_snooze();
      alarm_min=alarm_min+snooze_min;
      check_alarm();
      tone(buzzer, alarm_tone);
      delay(300);
      noTone(buzzer);
      delay(300);
      l.clear();
      l.print("Snoozing after ");l.print(snooze_min);l.print(" min");
      delay(1000);
      return;
    }
  }
...
if (buttonPressed(SW1)) {
  ...
  if (a > 1) {
    ...
    l.print("SAVED");
    l.setCursor(0, 1);
    l.print("1.set 4.no set");////xhere the prev mins is display
    //////////////////
    delay(1000);
    sw = 0;     
   } //user to set snooze time 
   //delay(20);
...
void set_snooze() {
  ...
  while (true) {
    l.setCursor(0,1);
    l.print("MIN: ");
    if (snooze_min < 10) l.print("0");
    l.print(snooze_min);
    l.print("   ");

    if (buttonPressed(SW2)) {
      snooze_min++;
      if (snooze_min > 30) snooze_min = 1;
    }

    if (buttonPressed(SW3)) {
      snooze_min--;
      if (snooze_min < 1) snooze_min = 30;
    }
